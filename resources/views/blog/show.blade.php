...
<div class="flex flex-col w-full">
    <article class="min-h-screen bg-white w-full">
        <!-- Article Header -->
        <header class="bg-gray py-16 md:py-24 w-full">
            <div class="container mx-auto px-4 md:px-6">
                <div class="text-center">
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-6 text-muted-foreground">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div class="lg:col-span-3">
                    @if($post->featured_image)
                        <div class="mb-8 rounded-xl overflow-hidden shadow-lg border border-gray-200">
                            <img src="{{ asset('storage/' . $post->featured_image) }}" alt="{{ $post->title }}" class="w-full h-64 md:h-96 object-cover">
                        </div>
                    @endif

                    <!-- Article Body -->
                    <div class="prose prose-lg max-w-none prose-headings:font-bold prose-a:text-primary hover:prose-a:text-highlight prose-img:rounded-xl prose-blockquote:border-l-primary prose-blockquote:bg-primary/10 prose-blockquote:py-1 prose-p:text-muted-foreground prose-headings:text-dark prose-strong:text-dark prose-li:text-muted-foreground prose-blockquote:text-muted-foreground">
                    ...
                    <div class="mt-12 pt-8 border-t border-gray-200">
                        <div class="flex flex-wrap items-center justify-between gap-4">
                            <div class="flex items-center space-x-2 text-sm text-muted-foreground">
                                <span>Posted in</span>
                                <span class="bg-primary/20 text-dark px-3 py-1 rounded-full">General</span>
                            </div>
                            <div class="flex space-x-4">
                                <!-- Social Sharing -->
                                <button onclick="shareOnTwitter()" class="text-gray-400 hover:text-primary transition-colors duration-200">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"/>
                                    </svg>
                                </button>
                                <button onclick="shareOnLinkedIn()" class="text-gray-400 hover:text-primary transition-colors duration-200">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="flex items-start justify-between mb-4">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-muted-foreground">{{ $comment->created_at->diffForHumans() }}</p>
                                            </div>
                                        </div>
                                        @auth
                            ...
                            <form action="{{ route('blog.comments.store', $post) }}" method="POST">
                                ...
                                @guest
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            <label for="guest_name" class="block text-sm font-medium mb-2">Name *</label>
                                            <input 
                                                type="text" 
                                                name="guest_name" 
                                                id="guest_name"
                                        ...
                                        <div>
                                            <label for="guest_email" class="block text-sm font-medium mb-2">Email *</label>
                                            <input 
                                                type="email" 
                                                name="guest_email" 
                                                id="guest_email"
                                ...
                                <button 
                                    type="submit" 
                                    class="w-full md:w-auto px-6 py-3 bg-primary text-dark hover:bg-highlight font-medium rounded-md transition-colors"
                                >
                                    Post Comment
                                </button>
                            </form>
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="bg-gray border border-gray-200 rounded-xl p-6 mb-6">
                        ...
                        <div class="flex items-center space-x-3 mb-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-muted-foreground">Community Contributor</p>
                            </div>
                        </div>
                    ...
                    <div class="bg-gray border border-gray-200 rounded-xl p-6">
                        ...
                        <div class="flex space-x-3">
                            <button onclick="shareOnTwitter()" class="flex-1 flex items-center justify-center px-4 py-2 bg-primary text-dark hover:bg-highlight font-medium rounded-md transition-colors">
                                <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"/>
                                </svg>
                                Twitter
                            </button>
                            <button onclick="shareOnLinkedIn()" class="flex-1 flex items-center justify-center px-4 py-2 bg-primary text-dark hover:bg-highlight font-medium rounded-md transition-colors">
                                <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                                </svg>
                                LinkedIn
